...
# Generate routes and OpenAPI spec (or use: bun run build)
bunx tsoa spec
bunx tsoa routes
...
- OpenAPI Spec: `http://localhost:9000/public/openapi.json`
...
curl -X POST http://localhost:9000/todos \
  -H "Content-Type: application/json" \
  -d '{"user_id": 1, "title": "Learn TSOA", "description": "Study the docs"}'
...
For detailed API documentation, see [API.md](API.md) or visit the Swagger UI at `http://localhost:9000/docs`.
